...
def run_omniCLIP(args):
    ...
    OutFile = os.path.join(
        EmissionParameters["out_dir"], EmissionParameters["out_file_base"] + ".txt"
    )
...
def PerformIteration(
    Sequences, Background, IterParameters, NrOfStates, First, NewPaths={}, verbosity=1
):
    ...
    NewEmissionParameters = FitEmissionParameters(
        Sequences, Background, NewPaths, EmissionParameters, First, verbosity=verbosity
    )
    ...
    TransistionPredictors = trans.FitTransistionParameters(
        Sequences, Background, TransitionParameters, NewPaths, verbosity=verbosity
    )
...
def add_pseudo_gene(Sequences, Background, NewPaths, PriorMatrix):
    ...
    random_gen = np.random.choice(
        np.arange(len(gen_lens)), 1, p=np.array(gen_lens) / np.float(sum(gen_lens))
    )
...
def main():
    ...
    parser_run_omniCLIP_reqNamed.add_argument(
        "--out-dir", action="store", dest="out_dir", help="Output directory for results"
    )
    parser_run_omniCLIP_reqNamed.add_argument(
        "--db-file",
        action="store",
