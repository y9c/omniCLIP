...
def PredictTransistions(
    Counts, TransitionParameters, NrOfStates, Type="multi", verbosity=1
):
        ...
        if CurrentState == 0:
            ...
            if np.sum(ix_nonzero) > 0:
                ...
                NormFactor[ix_nonzero] = logsumexp(
                    TransistionProb[:, CurrentState, 1:][:, ix_nonzero], axis=0
                )
...
def FitTransistionParametersSimple(
    Sequences, Background, TransitionParameters, CurrPath, verbosity=1
):
    """Determine optimal logistic regression parameters.
