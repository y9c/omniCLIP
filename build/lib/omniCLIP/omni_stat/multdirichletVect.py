...
def log_pdf_vect(k, alpha):
    ...
    if len(k.shape) == 1:
        ...
        Ks = k.shape[1]
    Pos = (
        gammaln(np.sum(k, axis=0) + 1)
        + np.tile(gammaln(np.sum(alpha)), (1, Ks))
        + np.sum(gammaln(np.tile(alpha, (1, Ks)) + k), axis=0)
    )
    Neg = (
        np.sum(gammaln(k + 1), axis=0)
        + gammaln(np.sum(np.tile(alpha, (1, Ks)) + k, axis=0))
        + np.tile(np.sum(gammaln(alpha)), (1, Ks))
    )
    log = Pos - Neg
...
def log_pdf_vect_rep(Counts, alpha, tracks_per_rep, NrOfReplicates):
    ...
    for rep in range(1, NrOfReplicates):
        new_k, Ks = expand_k(
            Counts[rep * tracks_per_rep : (rep + 1) * tracks_per_rep, :]
        )
        k += new_k

    # Compute the factors that are independent of the replicates
    Pos = np.sum(gammaln(k + np.tile(alpha, (1, Ks))), axis=0) + np.tile(
        gammaln(np.sum(alpha)), (1, Ks)
    )
    Neg = np.tile(np.sum(gammaln(alpha)), (1, Ks)) + gammaln(
        np.sum(np.tile(alpha, (1, Ks)) + k, axis=0)
    )
...
def TwoBinomlog_pdf_vect(k1, k2, alpha):
    ...
    k = k1 + k2
    Pos = (
        gammaln(np.sum(k1, axis=0) + 1)
        + gammaln(np.sum(k2, axis=0) + 1)
        + np.tile(gammaln(np.sum(alpha)), (1, Ks))
        + np.sum(gammaln(np.tile(alpha, (1, Ks)) + k), axis=0)
    )
    Neg = (
        np.sum(gammaln(k1 + 1), axis=0)
        + np.sum(gammaln(k2 + 1), axis=0)
        + gammaln(np.sum(np.tile(alpha, (1, Ks)) + k, axis=0))
        + np.tile(np.sum(gammaln(alpha)), (1, Ks))
    )
    log = Pos - Neg
